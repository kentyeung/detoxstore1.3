Spree::BaseHelper.module_eval do
  def link_to_cart(text = nil)
    ...
    if current_order.nil? or current_order.item_count.zero?
      text = "(#{Spree.t('empty')})".html_safe
      css_class = 'empty cartcolor'
    else
      text = "(#{current_order.item_count})  <span class='amount'>#{current_order.display_total.to_html}</span>".html_safe
      css_class = 'full cartcolor'
    end
